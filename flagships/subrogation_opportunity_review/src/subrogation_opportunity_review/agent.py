...
def _build_client() -> ksapi.ApiClient:
    ...
    client.set_default_header(
        "Authorization", f"Bearer {os.environ['KS_API_KEY']}"
    )
    return client
...
def _do_list_contents(api: ksapi.ApiClient, folder_id: str) -> str:
    ...
    for raw in getattr(resp, "items", None) or []:
        ...
        pp_id = getattr(inner, "path_part_id", None) or getattr(
            inner, "id", None
        )
        rows.append(
            {
...
def _do_read(
    api: ksapi.ApiClient, path_part_id: str, max_chars: int
) -> str:
    pp = PathPartsApi(api).get_path_part(
        path_part_id=UUID(path_part_id)
    )
    ...
    if ptype == "CHUNK" and meta:
        ...
        return _truncate(
            getattr(chunk, "text", "") or "", max_chars
        )
    ...
    offset = 0
    while True:
        contents: Any = DocumentVersionsApi(
            api
        ...
        for item in items:
            ...
            text = (
                ...
                or getattr(inner, "content", "")
                or ""
            )
            cid = getattr(inner, "id", None) or getattr(
                inner, "metadata_obj_id", None
            )
            if text:
                pieces.append(
                    f"{text}"
                    f"{f' [chunk:{cid}]' if cid else ''}\n"
                )
        if len(items) < 100:
            break
        offset += 100
...
def _dispatch(
    api: ksapi.ApiClient, name: str, args: dict[str, Any]
) -> str:
...
def draft_review(
    ...
    *,
    ...
    return (
        "ERROR: agent exceeded max_steps without producing a "
        "final review."
    )
