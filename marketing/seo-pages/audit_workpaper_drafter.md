---
title: "Audit Workpaper Drafter agent — grounded in your docs, with citations"
description: "Accounting flagship — draft cited audit workpapers tying GL balances to source documents (LangGraph ReAct)."
keywords: ["audit workpaper drafter", "Accounting agent", "Accounting RAG", "document AI", "MCP"]
---

# Audit Workpaper Drafter

Accounting flagship — draft cited audit workpapers tying GL balances to source documents (LangGraph ReAct).
...
- Enumerates the `Accounting` corpus folder, reads relevant chunks, and produces a structured pydantic output.
...
## Why citations matter for Accounting

In Accounting, an unsourced AI answer is a liability. Knowledge Stack threads `[chunk:<uuid>]` markers through every read, and the schema rejects outputs that lack them. That's the difference between a demo and a control your auditors accept.

## Source

- Flagship: [`flagships/audit_workpaper_drafter/`](https://github.com/knowledgestack/ks-cookbook/tree/main/flagships/audit_workpaper_drafter)
