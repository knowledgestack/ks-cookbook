...
PROMPT = (
    "You triage inbound NDAs against the company policies. Use the MCP tools to read "
    "the data-classification, data-retention, vendor-management, and confidentiality "
    "policies from the folder "
    f"path_part_id={POLICIES_FOLDER}. For each clause flag, copy a real chunk_id from "
    "the read output into policy_ref.chunk_id. Set recommendation to 'accept' only if "
    "no high-severity flags exist."
)
...
async def run(counterparty: str, nda_text: str) -> None:
    ...
    agent = Agent(
        ...
        mcp_servers=[mcp], system_prompt=PROMPT, result_type=NDAReview,
    )
