...
class EscalationDecision(BaseModel):
    ticket_summary: str
    decision: str = Field(
        ..., pattern="^(hold|escalate_t2|escalate_t3|page_oncall|management_escalation)$"
    )
...
PROMPT = """You decide escalations vs the escalation matrix + SLA policy in
...
async def run(summary: str, severity: str, sla_min: int) -> None:
    ...
    agent = Agent(
        ...
        output_type=EscalationDecision,
        retries=4,
        output_retries=4,
    )
