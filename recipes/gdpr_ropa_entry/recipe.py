...
class ROPAEntry(BaseModel):
    ...
    lawful_basis: str = Field(
        ...,
        pattern="^(consent|contract|legal_obligation|vital_interest|public_task|legitimate_interest)$",
    )
...
PROMPT = """You draft Article 30 ROPA entries. Read privacy + data-retention +
...
def main() -> None:
    ...
    p.add_argument("--activity", required=True, help="e.g. 'customer support ticket handling'")
    args = p.parse_args()
